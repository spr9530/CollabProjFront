...
import { createTaskAsync } from '../task/TaskSlice';
...
function CreateTask({ visibility, setVisibility, roomInfo }) {
    ...
    const {
        register,
        handleSubmit,
        control, // used for managing array in form
        reset, // used for clearing that array
        formState: { errors }
    } = useForm();
    ...
    const createNewTask = async (data) => {
        ...
        try {
            await dispatch(createTaskAsync({ task, id1, id2: userInfo._id }));
    ...
    return (
        <div className={`absolute w-full h-screen flex flex-col justify-center items-center ${visibility}`}>
            <div className={`fixed left-1/2 transform -translate-x-1/2 top-1/2 -translate-y-1/2 bg-black rounded-md w-11/12 md:w-7/12 h-[98vh] max-h-[98vh] flex flex-col items-center p-3 overflow-scroll scrollbar-none`}>
                <form className='w-full' onSubmit={handleSubmit((data) => createNewTask(data))}>
                    <div className='text-xl text-white flex w-full justify-between mb-1'>
                        <div>New Task</div>
                        <div className='flex gap-2'>
                            <button className='bg-primaryGreen p-1 rounded-md' type='submit' disabled={loading}>
                                {loading ? 'Creating...' : 'Create'}
                            </button>
                            <button className='' type='button' onClick={() => setVisibility('hidden')}>
                    ...
                    <div className='w-full flex flex-col md:flex-row gap-2'>
                        <div className='w-full md:w-7/12 flex flex-col gap-1'>
                            <input
                        ...
                        <div className='w-full md:w-5/12 h-fit border-b-2 border-b-primaryBlue text-white'>
